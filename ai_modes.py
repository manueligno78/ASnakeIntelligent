...
def ai_play_snake():
    ...
    ga = GeneticAlgorithm(population_size=pop_size, hidden_size=hidden_layer_size, activation_function=activation_function, learning_rate=learning_rate, mutation_rate=mutation_rate)
    try:
...
def draw_network_structure(surface, neural_net, activations=None):
    if activations:
        ...
        layers = [getattr(neural_net, "input_size", 6), getattr(neural_net, "hidden_size", 10), getattr(neural_net, "output_size", 4)]
    ...
    for i, num_neurons in enumerate(layers):
        ...
        for j in range(num_neurons):
            y = (j + 1) * neuron_gap
            act = activations[i][j] if activations and i < len(activations) and j < len(activations[i]) else 0.5
            intensity = max(0, min(255, int(act * 255)))
            neuron_color = (intensity, 100, 255 - intensity)
